      ...
      users.forEach((user) => {
        if (!newPositions[user.username]) {
          newPositions[user.username] = {
            x: Math.random() * 90, // Limit to 90% to prevent overflow
            y: Math.random() * 90, // Limit to 90% to prevent overflow
          };
        }
      });
      return newPositions;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setDuckPositions((prevPositions) => {
        const newPositions = { ...prevPositions };
        users.forEach((user) => {
          if (!newPositions[user.username]) return;

          newPositions[user.username] = {
            x: Math.max(0, Math.min(90, newPositions[user.username].x + (Math.random() - 0.5) * 20)),
            y: Math.max(0, Math.min(90, newPositions[user.username].y + (Math.random() - 0.5) * 20)),
          };
        });
        return newPositions;
      });
