export const MINIGAMES = [
  {
    type: 'votingquestion',
    name: 'Group Vote',
    description: 'Vote on fun questions about the group!',
    duration: 15,
    maxScore: 10,
    instruction: 'Choose someone from the group!'
  },
  {
    type: 'quiz',
    name: 'Quick Quiz',
    description: 'Test your knowledge with multiple choice questions!',
    duration: 20,
    maxScore: 10,
    instruction: 'Choose the correct answer!'
  },
  {
    type: 'buttonmash',
    name: 'Button Masher',
    description: 'Mash the button as fast as you can!',
    duration: 5,
    maxScore: 100,
    instruction: 'Click the button as many times as possible!'
  },
  {
    type: 'quickmath',
    name: 'Quick Math',
    description: 'Solve math problems quickly!',
    duration: 20,
    maxScore: 15,
    instruction: 'Solve the math problems as fast as you can!'
  },
  {
    type: 'colorclick',
    name: 'Color Match',
    description: 'Click the correct color as fast as you can!',
    duration: 15,
    maxScore: 20,
    instruction: 'Click the color that matches the text!'
  },
  {
    type: 'typespeed',
    name: 'Speed Typer',
    description: 'Type the words as fast as you can!',
    duration: 15,
    maxScore: 20,
    instruction: 'Type the words exactly as shown!'
  },
  {
    type: 'memorymatch',
    name: 'Memory Match',
    description: 'Match the pairs of cards!',
    duration: 15,
    maxScore: 6,
    instruction: 'Find all matching pairs before time runs out!'
  },
  {
    type: 'reactiontime',
    name: 'Reaction Time',
    description: 'Test your reflexes!',
    duration: 15,
    maxScore: 20,
    instruction: 'Click as soon as the color changes to green!'
  },
  {
    type: 'wordscramble',
    name: 'Word Scramble',
    description: 'Unscramble the words!',
    duration: 20,
    maxScore: 15,
    instruction: 'Rearrange the letters to form the correct word!'
  },
  {
    type: 'sequencerepeat',
    name: 'Sequence Repeat',
    description: 'Remember and repeat the sequence of colors!',
    duration: 20,
    maxScore: 15,
    instruction: 'Watch the sequence and repeat it in the correct order!'
  },
  {
    type: 'fallingcatch',
    name: 'Falling Catch',
    description: 'Catch the falling objects!',
    duration: 15,
    maxScore: 20,
    instruction: 'Move the basket to catch the falling items!'
  },
  {
    type: 'targetshoot',
    name: 'Target Shoot',
    description: 'Hit the moving targets!',
    duration: 15,
    maxScore: 20,
    instruction: 'Click on the targets before they disappear!'
  },
  {
    type: 'horseracing',
    name: 'Horse Racing',
    description: 'Bet on horses and win big!',
    duration: 100,
    maxScore: 100,
    instruction: 'Place your bet on a horse and watch the race!'
  },
  {
    type: 'roulette',
    name: 'Roulette',
    description: 'Place your bets on the roulette wheel!',
    duration: 300,
    maxScore: 100,
    instruction: 'Choose your bet type and amount!'
  }
];